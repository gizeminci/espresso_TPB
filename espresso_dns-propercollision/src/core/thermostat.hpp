...
inline void fluid_velocity(Particle *p, double vfxyz[3]){
    ...
    constant_vel= C1xy[0]*(box_l[0]/(2*nx[0]))*0.1;
    ...
    for (a=0; a<n; a++) {
        ...
        coefx[a] = (p->r.p[0]-0.00000000001-cx[a])/fabs((p->r.p[0]-0.00000000001-cx[a]));
        coefy[a] = (p->r.p[1]-0.00000000001-cy[a])/fabs((p->r.p[1]-0.00000000001-cy[a]));
        coefz[a] = (p->r.p[2]-0.00000000001-cz[a])/fabs((p->r.p[2]-0.00000000001-cz[a]));
        ...
        if (fabs(p->r.p[1]-cy[a])>fabs(p->r.p[0]-cx[a])) {
           weightxy = 1.-(sqrt((pow((p->r.p[0]-cx[a]),2)+pow((p->r.p[1]-cy[a]),2)-rxy[a]))/(sqrt(rxy[a]*rxy[a]+pow((p->r.p[0]-cx[a]),2))-rxy[a]));
        }else {
           weightxy = 1.-(sqrt((pow((p->r.p[0]-cx[a]),2)+pow((p->r.p[1]-cy[a]),2)-rxy[a]))/(sqrt(rxy[a]*rxy[a]+pow((p->r.p[1]-cy[a]),2))-rxy[a]));
        }

        if (fabs(p->r.p[2]-cz[a])>fabs(p->r.p[0]-cx[a])) {
           weightxz = 1.-(sqrt((pow((p->r.p[0]-cx[a]),2)+pow((p->r.p[2]-cz[a]),2)-rxz[a]))/(sqrt(rxz[a]*rxz[a]+pow((p->r.p[0]-cx[a]),2))-rxz[a]));
        }else {
           weightxz = 1.-(sqrt((pow((p->r.p[0]-cx[a]),2)+pow((p->r.p[2]-cz[a]),2)-rxz[a]))/(sqrt(rxz[a]*rxz[a]+pow((p->r.p[2]-cz[a]),2))-rxz[a]));
        }

        if (fabs(p->r.p[2]-cz[a])>fabs(p->r.p[1]-cy[a])) {
           weightyz = 1.-(sqrt((pow((p->r.p[1]-cy[a]),2)+pow((p->r.p[2]-cz[a]),2)-ryz[a]))/(sqrt(ryz[a]*ryz[a]+pow((p->r.p[1]-cy[a]),2))-ryz[a]));
        }else {
           weightyz = 1.-(sqrt((pow((p->r.p[1]-cy[a]),2)+pow((p->r.p[2]-cz[a]),2)-ryz[a]))/(sqrt(ryz[a]*ryz[a]+pow((p->r.p[2]-cz[a]),2))-ryz[a]));
        }
...
/*******************different shapes of noise */
      ...
      p->f.f[j] = langevin_pref1*(p->m.v[0]-(langevin_pref3*vfxyz[j]))*PMASS(*p) + langevin_pref2*(d_random()-0.5)*massf;
#elif defined (GAUSSRANDOMCUT)
