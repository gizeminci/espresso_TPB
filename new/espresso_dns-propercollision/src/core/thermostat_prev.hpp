...
inline void fluid_velocity(Particle *p, double vfxyz[3]){
    ...
    for (a=0; a<n; a++) {
        ...
        rxy[a] = sqrt(SQR(p->r.p[0]-cx[a]-0.0001)+SQR(p->r.p[1]-cy[a]-0.0001));
        rxz[a] = sqrt(SQR(p->r.p[0]-cx[a]-0.0001)+SQR(p->r.p[2]-cz[a]-0.0001));
        ryz[a] = sqrt(SQR(p->r.p[1]-cy[a]-0.0001)+SQR(p->r.p[2]-cz[a]-0.0001));
	...
	coefx[a] = (p->r.p[0]-cx[a]-0.0001)/fabs((p->r.p[0]-cx[a]-0.0001));
	coefy[a] = (p->r.p[1]-cy[a]-0.0001)/fabs((p->r.p[1]-cy[a]-0.0001));
	coefz[a] = (p->r.p[2]-cz[a]-0.0001)/fabs((p->r.p[2]-cz[a]-0.0001));
